...
echo "#!/bin/bash" >> svd
echo "" >> svd
echo "yt-dlp -f 'bv+ba' -o \"%(title)s\" \"\$@\"" >> svd

echo "#!/bin/bash" >> mvd
echo "" >> mvd
echo "yt-dlp -f 'bv+ba' -o \"%(playlist_index)s-%(title)s\" \"\$@\"" >> mvd

echo "#!/bin/bash" >> svd-mp4
echo "" >> svd-mp4
echo "yt-dlp -f 'bv[ext=mp4]+ba[ext=m4a]' -o \"%(title)s\" \"\$@\"" >> svd-mp4

echo "#!/bin/bash" >> mvd-mp4
echo "" >> mvd-mp4
echo "yt-dlp -f 'bv[ext=mp4]+ba[ext=m4a]' -o \"%(playlist_index)s-%(title)s\" \"\$@\"" >> mvd-mp4
